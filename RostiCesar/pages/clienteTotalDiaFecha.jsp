		...
		if(session.getId().equals(request.getParameter("sessionid"))){//person is logged in, so let him/her in.
            ...
            <%@ include file="../admin/Body.jsp" %>
          	...
          	<div id="contentbox">
    			...
    			<form method="post" action="/RostiCesar/pages/clienteTotalDiaFecha.jsp?sessionid=<%= session.getId() %>">
    			...
    			<% if (!start.equals("") && !end.equals("")) { %>
    				...
    				<input type="submit" value="Vista Preeliminar" onclick=" return abrirVentana('<%= "&reporte=clienteTotalDiaFecha" + "&start=" + start + "&end=" + end %>');">
	    		 <% } %>    			    			 
    		</div>
