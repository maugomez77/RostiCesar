...
import mx.com.rosti.mappers.ProductoMapper;
 ...
 */
public class ProductoDAOImpl extends Conexiones implements ConsultasDAO {
    /**
     ...
     */
    private ProductoMapper pMAPPER = new ProductoMapper();
    /**
    ...
    public List<Producto> consultar() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Producto cl = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cl);
            }
    ...
    public Object consultarEspecifica(final Object pObj) {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        Producto cc = (Producto) pObj;
        try {
            conn = takeConnection(local, dataSource);
            ps = conn.prepareStatement(accesoComun.getFindUnique());
            ps.setString(1, cc.getIdProducto());
            ps.setInt(2, cc.getIdProveedor());
            rs = ps.executeQuery();
            while (rs.next()) {
     ...
     * @return List      Lista
     */
    public List<Producto> consultarRango(final Object[] pValues) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<Producto> list = new ArrayList<Producto>();
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Producto cc = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cc);
            }
        } catch (SQLException e) {
            Utilerias.error(this, e.toString());
     ...
     * @return List      Lista
     */
    public List<Producto> consultarPorDesc(String sql) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<Producto> list = new ArrayList<Producto>();
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Producto cc = pMAPPER.mapRowDesc(rs, rs.getRow());
                list.add(cc);
            }
        } catch (SQLException e) {
            Utilerias.error(this, e.toString());
    ...
    public List<ProductoAcumulado> consultarAcumulados(String sql) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<ProductoAcumulado> list = new ArrayList<ProductoAcumulado>();
        try {
            conn = takeConnection(local, dataSource);
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	ProductoAcumulado cc = pMAPPER.mapRowAcumulado(rs, rs.getRow());
                list.add(cc);
            }
        } catch (SQLException e) {
            Utilerias.error(this, e.toString());
     ...
     */
    public int dmlOperations(final String operacion, final Producto cc) {
        int result = -1;
