...
import mx.com.rosti.mappers.RolesMapper;
 ...
 */
public class RolesDAOImpl extends Conexiones implements ConsultasDAO {
    /**
     ...
     */
    private RolesMapper pMAPPER = new RolesMapper();
    ...
    public List<Roles> consultar() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Roles cl = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cl);
            }
     ...
     * @return List      Lista
     */
    public List<Roles> consultarRango(final Object[] pValues) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<Roles> list = new ArrayList<Roles>();
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Roles cc = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cc);
            }
        } catch (SQLException e) {
            Utilerias.error(this, e.toString());
     ...
     */
    public int dmlOperations(final String operacion, final Roles cc) {
        int result = -1;
