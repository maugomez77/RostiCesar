...
import mx.com.rosti.mappers.ClientesMapper;
 ...
 */
public class ClientesDAOImpl extends Conexiones implements ConsultasDAO {
    /**
     ...
     */
    private ClientesMapper pMAPPER = new ClientesMapper();
    ...
    public List<Clientes> consultar() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Clientes cl = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cl);
            }
     ...
     * @return List      Lista
     */
    public List<Clientes> consultarRango(final Object[] pValues) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<Clientes> list = new ArrayList<Clientes>();
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Clientes cc = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cc);
            }
        } catch (SQLException e) {
            Utilerias.error(this, e.toString());
     ...
     */
    public int dmlOperations(final String operacion, final Clientes cc) {
        int result = -1;
