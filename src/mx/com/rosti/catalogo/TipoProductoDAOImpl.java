...
import mx.com.rosti.mappers.TipoProductoMapper;
 ...
 */
public class TipoProductoDAOImpl extends Conexiones implements ConsultasDAO {
    /**
     ...
     */
    private TipoProductoMapper pMAPPER = new TipoProductoMapper();
    /**
    ...
    public List<TipoProducto> consultar() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	TipoProducto cl = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cl);
            }
    ...
    public Object consultarEspecifica(final Object pObj) {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        TipoProducto cc = (TipoProducto) pObj;
        try {
            conn = takeConnection(local, dataSource);
            ps = conn.prepareStatement(accesoComun.getFindUnique());
            ps.setInt(1, cc.getIdTipoProducto());
            rs = ps.executeQuery();
            while (rs.next()) {
     ...
     * @return List      Lista
     */
    public List<TipoProducto> consultarRango(final Object[] pValues) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<TipoProducto> list = new ArrayList<TipoProducto>();
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	TipoProducto cc = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cc);
            }
        } catch (SQLException e) {
            Utilerias.error(this, e.toString());
     ...
     */
    public int dmlOperations(final String operacion, final TipoProducto cc) {
        int result = -1;
