...
import mx.com.rosti.mappers.InventarioMapper;
 ...
 */
public class InventarioDAOImpl extends Conexiones implements ConsultasDAO {
    /**
     ...
     */
    private InventarioMapper pMAPPER = new InventarioMapper();
    /**
    ...
    public List<Inventario> consultar() {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Inventario cl = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cl);
            }
    ...
    public Object consultarEspecifica(final Object pObj) {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        Inventario cc = (Inventario) pObj;
        try {
            conn = takeConnection(local, dataSource);
            ps = conn.prepareStatement(accesoComun.getFindUnique());
            ps.setString(1, cc.getIdInventario());
            ps.setString(2, cc.getIdProducto());
            ps.setInt(3, cc.getIdProveedor());
            rs = ps.executeQuery();
            while (rs.next()) {
     ...
     * @return List      Lista
     */
    public List<Inventario> consultarRango(final Object[] pValues) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        List<Inventario> list = new ArrayList<Inventario>();
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	Inventario cc = pMAPPER.mapRow(rs, rs.getRow());
                list.add(cc);
            }
        } catch (SQLException e) {
            Utilerias.error(this, e.toString());
    ...
    public int insert(final Object pObj) {
        ...
        try {
            ...
            + " insert into inventario (id_inventario, cantidad, entrada_salida, fecha_entrada, "
            + "                         fecha_salida, hora_entrada, hora_salida, id_producto, " 
            + "                         id_proveedor, id_entrada_carga, id_salida_carga, remision, " 
            + "                         id_cliente, precio_unitario, cantidad_pagada) " 
            + "      values ('" + cc.getIdInventario() + "'" + ","
            + "               " + cc.getCantidad() + "," 
            + "              '" + cc.getEntradaSalida() + "'" + "," 
            + "              '" + Utilerias.getDate(cc.getFechaEntrada(), "yyyy-MM-dd") + "'" + ","
            + "              '" + Utilerias.getDate(cc.getFechaSalida(), "yyyy-MM-dd") + "'" + ","
            + "              '" + Utilerias.getTime(cc.getHoraEntrada(), "HH:mm:ss") + "'" + ","
            + "              '" + Utilerias.getTime(cc.getHoraSalida(), "HH:mm:ss") + "'" + ","
            + "              '" + cc.getIdProducto() + "'" + ","
            + "               " + cc.getIdProveedor() + ","
            + "               " + cc.getIdEntradaCarga() + ","
            + "               " + cc.getIdSalidaCarga() + ","
            + "               " + cc.getRemision() + ","
            + "               " + cc.getIdCliente()+ ", 0, 0 " + ")").getBytes(), "UTF-8");            
            
            Utilerias.debug(this, "Insercion: " + insert);
    ...
    public int update(final Object pObj) {
        ...
        try {
            ...
            + " update inventario set cantidad = " + cc.getCantidad() 
            + ", entrada_salida = '" + cc.getEntradaSalida() + "'" 
            + ", id_salida_carga = " + cc.getIdSalidaCarga()
            + ", remision = " + cc.getRemision()
            + ", id_cliente = " + cc.getIdCliente()
            + ", hora_salida = '" + Utilerias.getTime(cc.getHoraSalida(), "HH:mm:ss") + "'"
            + ", fecha_salida = '" + Utilerias.getDate(cc.getFechaSalida(), "yyyy-MM-dd") + "'"
            + "  where id_inventario = '" + cc.getIdInventario() + "'" 
            + "    and id_producto = '" + cc.getIdProducto() + "'"  
            + "    and id_proveedor = " + cc.getIdProveedor()).getBytes(), "UTF-8");
            
            Utilerias.debug(this, "Update: " + update);
     ...
     */
    public int dmlOperations(final String operacion, final Inventario cc) {
        int result = -1;
